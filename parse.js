...
const text = `Tình huống 1: Người đi bộ sau xe tải
...
for (const line of lines) {
  if (line.startsWith('Tình huống')) {
    ...
    const match = line.match(/Tình huống (\d+):\s*(.*)/);
    if (match) {
      current = {
        id: parseInt(match[1]),
        name: match[2],
    ...
    if (current) current.indirect = line.replace('Dấu hiệu gián tiếp:', '').trim();
  } else if (line.startsWith('Dấu hiệu trực tiếp:')) {
    if (current) current.direct = line.replace('Dấu hiệu trực tiếp:', '').trim();
  } else if (line.startsWith('Gợi ý xử lý:')) {
    if (current) current.action = line.replace('Gợi ý xử lý:', '').trim();
  }
}
if (current) situations.push(current);
